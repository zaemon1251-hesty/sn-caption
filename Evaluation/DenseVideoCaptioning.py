"""
Copy from SoccerNet.Evaluation.DenseVideoCaptioning
"""
...
class DVCEvaluator(object):
    ...
    def __init__(
        ...
        SoccerNet_path,
        Predictions_path,
        prediction_file="results_caption.json",
        label_files="Labels-caption.json",
        tious=None,
        max_proposals=1000,
        split="test",
        version=6,
        ...
        for game in self.list_games:
            ...
            half_1, half_2 = self.load_file(
                root=SoccerNet_path,
                file=os.path.join(game, label_files),
                label_or_prediction=True,
            )
            self.ground_truths[game, 1] = dict(
                zip(["timestamps", "sentences"], zip(*half_1))
            )
            self.ground_truths[game, 2] = dict(
                zip(["timestamps", "sentences"], zip(*half_2))
            )

            half_1, half_2 = self.load_file(
                root=Predictions_path,
                file=os.path.join(game, prediction_file),
                label_or_prediction=False,
            )
            # TOCHANGE !!!!!!!!!!!!!!!!
            # path = os.path.join(SoccerNet_path, game, labels)
            # half_1, half_2 = self.load_file(path=path, label_or_prediction=True)
            self.prediction[game, 1] = [
                {"timestamp": list(timestamp), "sentence": caption}
                for timestamp, caption in half_1
            ]
            self.prediction[game, 2] = [
                {"timestamp": list(timestamp), "sentence": caption}
                for timestamp, caption in half_2
            ]
    ...
    def load_file(self, root: str, file: str, label_or_prediction: bool):
        ...
        for annotation in data["annotations" if label_or_prediction else "predictions"]:
            ...
            start, end = (
                time - self.window_size // 2,
                time + self.window_size // 2 + self.window_size % 2,
            )
            ...
            if half == 1:
                half_1.append(
                    (
                        (start, end),
                        annotation["anonymized" if label_or_prediction else "comment"],
                    )
                )
            else:
                half_2.append(
    ...
    def preprocess(self):
        ...
        for i, vid in enumerate(self.gt_vids):
            cur_preds.update(
                {
                    j + p_spliter[-1]: [{"caption": remove_nonascii(p["sentence"])}]
                    for j, p in enumerate(self.prediction[vid])
                }
            )
            ...
            for n in range(n_ref):
                ...
                cur_gts[n].update(
                    {
                        j + g_spliter[n][-1]: [{"caption": remove_nonascii(p)}]
                        for j, p in enumerate(self.ground_truths[n][vid]["sentences"])
                    }
                )
                g_spliter[n].append(
                    g_spliter[n][-1] + len(self.ground_truths[n][vid]["sentences"])
                )
        ...
        for i, vid in enumerate(self.gt_vids):
            ...
            for n in range(n_ref):
                ...
                _g = [
                    tokenize_gts[n][j]
                    for j in range(g_spliter[n][i], g_spliter[n][i + 1])
                ]
    ...
    def evaluate_activitynet(self):
        ...
        scores = {
            f"{metric}": np.mean(score_tiou) for metric, score_tiou in scores.items()
        }
        scores["Recall"] = []
        scores["Precision"] = []
        for tiou in self.tious:
            ...
            # valid tIoU overlaps.
                ...
                for pred in self.prediction[vid_id]:
                    ...
                    for gt in self.ground_truths:
                        ...
                        for caption_idx, caption_timestamp in enumerate(
                            gt_captions["timestamps"]
                        ):
                            if self.iou(pred["timestamp"], caption_timestamp) > tiou:
                                ...
                                cur_gts[unique_index] = [
                                    {
                                        "caption": remove_nonascii(
                                            gt_captions["sentences"][caption_idx]
                                        )
                                    }
                                ]
    ...
    def soda_c(self, iou, scores):
        max_score = {
            metric: self.chased_dp_assignment(iou * value)[0]
    ...
    def calc_iou_matrix(self, preds, golds):
        return np.array(
            [
                [self.iou(pred, ct) for pred in preds["timestamps"]]
                for ct in golds["timestamps"]
            ]
        )
    ...
    def evaluate_soda(self):
        ...
        for vid in tqdm(self.gt_vids):
            ...
            best_idx = {
                metric: np.argmax(value, axis=1) for metric, value in _f.items()
            }
