...
from util import evaluate as evaluate_spotting
...
def trainer(
    ...
    for epoch in range(max_epochs):
        ...
        # train for one epoch
        loss_training = train(
            phase, train_loader, model, criterion, optimizer, epoch + 1, train=True
        )

        # evaluate on validation set
        loss_validation = train(
            phase, val_loader, model, criterion, optimizer, epoch + 1, train=False
        )
...
def test_captioning(
    ...
    output_results = os.path.join(
        "models", model_name, f"results_dense_captioning_{split}.zip"
    )
