...
from utils import evaluate_commentary_spotting
...
def trainer(
    ...
    os.makedirs(
        os.path.join(os.environ.get("MODEL_DIR"), "models", model_name, phase),
        exist_ok=True,
    )
    for epoch in range(max_epochs):
        best_model_path = os.path.join(
            os.environ.get("MODEL_DIR"), "models", model_name, phase, "model.pth.tar"
        )
        ...
        os.makedirs(
            os.path.join(os.environ.get("MODEL_DIR"), "models", model_name),
            exist_ok=True,
        )
    ...
    with tqdm(enumerate(dataloader), total=len(dataloader)) as t:
        for batch_idx, batch in t:
            ...
            if phase == "spotting":
                ...
                ), f"output shape ({output.shape}) != labels shape ({labels.shape})"

                # hand written NLL criterion
                loss = criterion(labels, output) / accumulation_steps

                feats_size = feats.size(0)
            elif phase == "caption":
                ...
                mask = pack_padded_sequence(
                    mask[:, 1:], lengths, batch_first=True, enforce_sorted=False
                )[0]
                feats = feats.cuda()
                # compute output
                output = model(feats, caption, lengths) / accumulation_steps
            ...
            losses.update(loss.item(), feats_size)
...
def validate_spotting(dataloader, model, model_name):
    ...
    AP = []
    for class_i in range(1, dataloader.dataset.num_classes + 1):
        AP.append(
...
def test_spotting(
    ...
    output_results = os.path.join(
        os.environ.get("MODEL_DIR"), "models", model_name, output_folder
    )
    ...
    _, _, _, inv_dict = getMetaDataTask(
        "caption", "SoccerNet", dataloader.dataset.version
    )
    ...
    with tqdm(enumerate(dataloader), total=len(dataloader)) as t:
        for i, (game_ID, feat_half1, feat_half2, label_half1, label_half2) in t:
            ...
            for b in range(int(np.ceil(len(feat_half1) / BS))):
                start_frame = BS * b
                end_frame = (
                    BS * (b + 1) if BS * (b + 1) < len(feat_half1) else len(feat_half1)
                )
            ...
            for b in range(int(np.ceil(len(feat_half2) / BS))):
                start_frame = BS * b
                end_frame = (
                    BS * (b + 1) if BS * (b + 1) < len(feat_half2) else len(feat_half2)
                )
            ...
            # Remove the no utterance label 次元を取り除き、行列からベクトルにする
            ...
            for half, timestamp in enumerate(
                ...
                for class_i in range(dataloader.dataset.num_classes):
                    spots = get_spot(
                        timestamp[:, class_i],
                    ...
                    for spot in spots:
                        ...
                        prediction_data["label"] = inv_dict[class_i]
    ...
    tight = {f"{k}_tight": v for k, v in tight.items() if v is not None}
    loose = {f"{k}_loose": v for k, v in loose.items() if v is not None}
    medium = {f"{k}_medium": v for k, v in medium.items() if v is not None}
...
def test_commentary_spotting(
    ...
    with tqdm(enumerate(dataloader), total=len(dataloader)) as t:
        for i, (game_ID, feat_half1, feat_half2, label_half1, label_half2) in t:
            ...
            def get_spot_from_NMS_cross_class(Input, window=60, thresh=0.0):
                """
                最大要素周辺のspot削除処理を、class横断的に行う
                """
                num_frames, num_classes = Input.shape
                detections_tmp = np.copy(Input)
                indexes = []
                max_values = []
                class_indexes = []
                ...
                while np.max(detections_tmp) >= thresh:
                    # Get the global max index, value, and corresponding class
                    max_value = np.max(detections_tmp)
                    max_index = np.unravel_index(
                        np.argmax(detections_tmp), detections_tmp.shape
                    )
                    max_frame_index = max_index[0]
                    max_class_index = max_index[1]

                    max_values.append(max_value)
                    indexes.append(max_frame_index)
                    class_indexes.append(max_class_index)

                    # Apply suppression across all classes in the window around the detected maximum
                    nms_from = int(np.maximum(-(window / 2) + max_frame_index, 0))
                    nms_to = int(
                        np.minimum(max_frame_index + int(window / 2), num_frames)
                    )
                    detections_tmp[nms_from:nms_to, :] = -1

                # Return the results as an array where each row contains [frame_index, class_index, max_value]
                return np.transpose([indexes, class_indexes, max_values])
            ...
            predictions_halfs = [
                np.zeros_like(timestamp_long_half_1),
                np.zeros_like(timestamp_long_half_2),
            ]
            ...
            for half, timestamp in enumerate(
                ...
                for class_i in range(num_classes):
                    ...
                    for spot in spots:
                        ...
                        # 予測値を保存し、evaluate_commentary_spottingに渡す
                        predictions_halfs[half][frame_index, class_i] = confidence
            ...
            # evaluate_commentary_spottingのためのラベル・予測値を格納する
            labels_list.append(label_half1)
            labels_list.append(label_half2)
            predictions_list.append(predictions_halfs[0])
            predictions_list.append(predictions_halfs[1])
    ...
    tight = evaluate_commentary_spotting(
        labels_list=labels_list,
        predictions_list=predictions_list,
        metric="tight",
    )

    loose = evaluate_commentary_spotting(
        labels_list=labels_list,
        predictions_list=predictions_list,
        metric="loose",
    )

    medium = evaluate_commentary_spotting(
        labels_list=labels_list,
        predictions_list=predictions_list,
        metric="medium",
    )
...
def validate_captioning(dataloader, model, model_name):
    ...
    scores = caption_scorer.compute_metrics(
        ref_list=[
            all_labels,
        ],
        hyp_list=all_outputs,
    )
    return scores
...
def test_captioning(
    ...
    output_results = os.path.join(
        ...
        "models",
        model_name,
        f"results_dense_captioning_{split}.zip",
    )
    ...
    results = {
        **{f"{k}_tight": v for k, v in tight.items()},
        **{f"{k}_loose": v for k, v in loose.items()},
    }
