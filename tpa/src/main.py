...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        description="SoccerNet-Caption: Dense Video Captioning for Soccer Broadcasts Commentaries",
    ...
    parser.add_argument(
        "--GPU", required=False, type=str, default="-1", help="IDs of the GPU to use"
    )
    parser.add_argument(
        "--max_num_worker",
        required=False,
    ...
    if args.first_stage == "spotting":
        ...
        args.weights_encoder = (
            (
                os.path.join(
                    os.environ.get("MODEL_DIR"),
                    f"models/{args.model_name}/spotting/model.pth.tar",
                )
            )
            if args.pretrain
            else None
        ...
        args.weights_encoder = (
            os.path.join(
                os.environ.get("MODEL_DIR"),
                (f"models/{args.model_name}/caption/model.pth.tar"),
            )
            if args.pretrain
            else None
        )
